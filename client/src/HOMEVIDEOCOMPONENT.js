...
const HOMEVIDEOCOMPONENT = (props) => {
    ...
    const METHOD = async () => {
        await axios.get(`/api/USERVIDEOVIDEO`)
            .then((RES) => {
                console.log(RES);
    ...
    const USERIMAGEDATA = async () => {
        await axios.get(`/api/USERIMAGEDATA`)
            .then((RES) => {
                console.log(RES);
                                ...
                                USERVIDEOLIST.filter((value) => {
                                    ...
                                    <li className="nav-item" key={index}><br />
                                        <div style={{ display: "flex", alignItems: "center" }}>
                                            <video src={`/api/${value.VIDEOFIVE}`} type="video/mp4" style={{ width: "30%", border: "5px solid white", marginLeft: "0%", marginTop: "0%" }} onClick={() => VIDEOFUNCTION(value.id, value.USERID, value.VIDEOONE, value.VIDEOTWO, value.VIDEOTHREE, value.VIDEOFIVE, value.VIDEONOISEREDUCE, value.VIDEOMUSIC, value.USERAUDIO)}></video>
                                            {
                                                USERIMAGE.map((val, i) => {
                                                    if (val.USERGENERATEDID === value.USERID) {
                                                        return (
                                                            <div key={i} style={{ marginLeft: "30%", marginTop: "-9%" }}>
                                                                <img className="user-img" src={`/api/${val.IMAGE}`} alt="User" style={{ borderRadius: "50%", width: "25px", height: "25px", marginLeft: "-80px" }} />
