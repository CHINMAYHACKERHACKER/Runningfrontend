...
const VIDEOUPLOAD = () => {
    ...
    function DELETEFUNCTION(ID) {
        axios.delete(`/api/USERVIDEODELETE/${ID}`);
        alert("Your Video Deleted");
    }

    useEffect(() => {
        axios.get(`/api/USERVIDEOVIDEO`).then((RES) => {
    ...
    useEffect(() => {
        axios.get(`/api/USERFOLLOWSTATUS`).then((RES) => {
    ...
    useEffect(() => {
        axios.get(`/api/USERCOMMENTBELLSTATUS`).then((RES) => {
    ...
    useEffect(() => {
        axios.get(`/api/USERVIEWDATA`).then((RES) => {
                    ...
                    if (likeCount !== 0 || commentCount !== 0 || viewCount !== 0) {
                        return (
                            <tbody key={index}>
                                <tr>
                                    <td>
                                        <video src={`/api/${value.VIDEOFIVE}`} type="video/mp4" style={{ width: "100%", maxWidth: "400px", margin: "0 auto" }}></video>
