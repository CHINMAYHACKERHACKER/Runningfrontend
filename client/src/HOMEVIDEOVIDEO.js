...
const HOMEVIDEOVIDEO = () => {
    ...
    function FULLWINDOWPOPUP(ID) {
        window.open(`https://comedytv.jprq.live/USERCOMMENT/${ID}`, "bfs", "fullscreen,scrollbars");
    }
    ...
    useEffect(() => {
        axios.get(`/api/USERVIDEOLISTINFORMATION`)
            .then((RES) => {
    ...
    if (PARAM.VIDEONOISEREDUCE == "yes") {
        const toggle = () => {
            videoRef.current.setVideoSrc(
                `/api/VIDEONOISEREDUCE/${PARAM.VIDEOFIVE}`
            );
        };
    }
    else {
                        ...
                        quality: [
                            {
                                name: "BD",
                                url:
                                    `/api/VIDEONOISEREDUCE/${PARAM.VIDEOONE}`
                            },
                            {
                                name: "FHD",
                                url:
                                    `/api/VIDEONOISEREDUCE/${PARAM.VIDEOTWO}`
                            },
                            {
                                name: "HD",
                                url:
                                    `/api/VIDEONOISEREDUCE/${PARAM.VIDEOTHREE}`
                            },
                            {
                                name: "SD",
                                url:
                        ...
                        quality: [
                            {
                                name: "BD",
                                url:
                                    `/api/VIDEO/${PARAM.VIDEOONE}`
                            },
                            {
                                name: "FHD",
                                url:
                                    `/api/VIDEO/${PARAM.VIDEOTWO}`
                            },
                            {
                                name: "HD",
                                url:
                                    `/api/VIDEO/${PARAM.VIDEOTHREE}`
                            },
                            {
                                name: "SD",
                                url:
                ...
                return USERVIDEOLIST.map((value, i) => {
                    if (val.USERGENERATEDID == value.USERID && value.VIDEOONE == `VIDEONOISEREDUCE/${PARAM.VIDEOONE}` && value.id == PARAM.ID) {
                        return <>
                            ...
                            <h6 key={i} style={{ marginTop: "-2%", marginLeft: "0%" }}>{val.USERNAME}</h6>   <img className="user-img" src={`/api/${val.IMAGE}`} alt="User" style={{ marginTop: "-6%", borderRadius: "50%", borderRadius: "50%", width: "50px", height: "50px" }} /><br />
