...
const VIDEO = () => {
    ...
    const ADMETHOD = async (id) => {
        await axios.get(`/api/ADVIDEO/${id}`)
            .then((RES) => {
                console.log(RES);
    ...
    useEffect(() => {
        axios.get(`/api/TOTALUSERCOMMENTBELLSTATUS`)
            .then((RES) => {
    ...
    useEffect(() => {
        axios.get(`/api/USERDATAFOLLOWSTATUS`)
            .then((RES) => {
            ...
            </div>
            {
                USERDATACRED.map((val) => {
                    if (USERLOGINDATA == val.USERGENERATEDID) {
                        return <div>
                            <img className="user-img" src={`/api/${val.IMAGE}`} alt="User" style={{ marginTop: "0%", borderRadius: "50%", width: "30px", height: "30px", marginLeft: "30%" }} />
                        </div>
                    }
                })
            }
